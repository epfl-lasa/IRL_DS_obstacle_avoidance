...
function [r,g,drdu,d2rdudu,drdx,d2rdxdx,gfull,Hfull] = ...
    ddistevalreward(reward,mdp_data,x,u,states,A,B,dxdu,d2xdudu)
...
r = reward.r * sum((u(:,[1,2])-u(1,:)).^2,2);

if nargout >= 2,
    % Compute gradient with respect to controls.
    g = zeros(T,Du);
    g(:,reward.idx) = 2.0 * reward.r * (u(:,[1,2])-u(1,:));
end;

if nargout >= 3,
    drdu = zeros(T,Du);
    drdu(:,reward.idx) = 2.0 * reward.r * (u(:,[1,2])-u(1,:));
...
if nargout >= 7,
    ...
    gfull = zeros(T,T*Du);
    for i=reward.idx,
        gfull(:,(i-1)*T + (1:T)) = diag(2.0*(u(:,i)-u(1,i))) * reward.r;
    end;
    % Compute Hessian.
